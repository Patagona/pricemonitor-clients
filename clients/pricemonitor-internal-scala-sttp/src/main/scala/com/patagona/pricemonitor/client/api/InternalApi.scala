  ...
  def createAuthToken(email: String, body: Option[Any] = None)(implicit basicAuth: Option[BasicCredentials], bearerToken: Option[BearerToken]): ApiRequestT[Any] =
{
   ...
   *   code 400 : ApiErrorResponse (Request validation failed. Common reasons include: - Invalid flag key format (must be alphanumeric, underscore, or dash) - Flag key already exists - Flag name is empty or too long - Description exceeds maximum length )
   * 
   * Available security schemes:
   *   BasicAuth (http)
   *   BearerAuth (http)
